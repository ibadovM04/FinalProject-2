using FinalProject.Areas.Admin.Models;
using FinalProject.Areas.Admin.ViewModels;
using FinalProject.Data;
using FinalProject.DTOs;
using FinalProject.Helper;
using FinalProject.Models;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.ViewEngines;
using Microsoft.EntityFrameworkCore;

namespace FinalProject.Areas.Admin.Controllers
{
    [Area("Admin")]
    public class CategoryController : Controller
    {
        private readonly ApplicationDbContext _context;
        private readonly IConfiguration _configuration;
        private ICompositeViewEngine _viewEngine;

        public CategoryController(ApplicationDbContext context,
                                         IConfiguration configuration,
                                         ICompositeViewEngine viewEngine)
        {
            _context = context;
            _configuration = configuration;
            _viewEngine = viewEngine;
        }
        [HttpGet]
        public async Task<IActionResult> List(int page = 1)
        {
            var query = _context.Categories.Where(u => true);

            var result = await SelectCategories(query, page);

            var vm = new CategoryListVm
            {
                CurrentPage = page,
                TotalPage = result.Item2,
                Categories = result.Item1
            };

            return View(vm);
        }
        public async Task<JsonResult> Filter([FromQuery] CategoryFilterModel request)
        {


            var query = _context.Categories.Where(u =>
                                           
                                            (String.IsNullOrEmpty(request.Name) || u.Name.ToLower().StartsWith(request.Name.ToLower())));




            var result = await SelectCategories(query, request.Page);


            var vm = new CategoryListVm
            {
                CurrentPage = request.Page,
                TotalPage = result.Item2,
                Categories = result.Item1
            };

            var html = await RenderPartialView.InvokeAsync("_CategoryListPartialView",
                                                            ControllerContext,
                                                            _viewEngine,
                                                            ViewData,
                                                            TempData,
                                                            vm);

            return Json(new
            {
                status = 200,
                data = html
            });
        }

        private async Task<(List<CategoryDto>, int)> SelectCategories(IQueryable<Category> query, int page)
        {
            var takeNumber = Convert.ToInt32(_configuration["List:AdminContacts"]);

            query = query.OrderByDescending(u => u.Created);

            var count = await query.CountAsync();

            var totalPage = (int)Math.Ceiling(count / (decimal)takeNumber);

            var categories = await query
                                 .Select(u => new CategoryDto
                                 {
                                     CategoryId=u.Id,
                                     Name=u.Name,
                                     Slogan=u.Slogan


                                 })
                                 .Skip((page - 1) * takeNumber)
                                 .Take(takeNumber)
                                 .ToListAsync();


            return (categories, totalPage);
        }
    }
}
